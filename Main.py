...
class App(customtkinter.CTk):
    ...
    def read_data(self):
        ...
        if getedinput == 'all':
            signalSample,fields = wfdb.rdsamp(ecg_path, warn_empty=True)        
            record = wfdb.rdrecord(ecg_path)
            annotation = wfdb.rdann(ecg_path, 'atr')
        else:    
            
            signalSample,fields = wfdb.rdsamp(ecg_path,sampto = int(getedinput), warn_empty=True)   
            record = wfdb.rdrecord(ecg_path, sampto=int(getedinput))
    ...
    def analize_signal(self):
        ...
        analyze.label_info_3 = customtkinter.CTkLabel(master=analyze.frame_right,
                                                   text=f"Liczba Próbek: {round(record.sig_len)}",
                                                   height=10,
                                                   corner_radius=6,  # <- custom corner radius
                                                   fg_color=("white", "gray38"),  # <- custom tuple-color
                                                   ...
                                                   anchor="w",
                                                   text_font=("Trebuchet MS",12))
        analyze.label_info_3.grid(column=1, row=2, sticky="nwe", padx=5, pady=5)
        ...
        analyze.label_info_3 = customtkinter.CTkLabel(master=analyze.frame_right,
                                                   ...
                                                   anchor="w",
                                                   text_font=("Trebuchet MS",12))
        analyze.label_info_3.grid(column=1, row=3, sticky="nwe", padx=5, pady=5)
